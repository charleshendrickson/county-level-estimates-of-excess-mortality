...
```{r}
dat <- read.csv(here('raw_data/acm_2009-2018.csv')) %>% 
...
dat_est <- dat_est %>% filter(year < 2018)
```
...
```{r}
if(1 == 1){
  save(fit, file = here('rda_2018/estimated_poisson_glm_parameters.rda'))
}
```
...
if(1 == 1){
  save(clust_ses, file = here('rda_2018/estimated_poisson_glm_ses.rda'))
}
```
...
dat_2018 <- dat_2018 %>%
  mutate(fitted_deaths_2018 = exp(predict.glm(fit, dat_2018)),
         fitted_death_rate_2018 = fitted_deaths_2018 / (exposure))
